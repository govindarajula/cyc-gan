...
class cycleGAN(object):
    ...
    def train(self,args): #transforming i/p image
        ...
        for epoch in range(self.start_epoch, args.epochs):
            ...
            for i, (a_real, b_real) in enumerate(zip(a_loader, b_loader)):
                ...
                b_idt_loss = self.L1(b_idt, b_real) * args.lamda * args.idt_coef #identity losses


                a_fake_dis = self.Da(a_fake)
                b_fake_dis = self.Db(b_fake) # adversarial losses
                ...
                gen_loss = a_gen_loss + b_gen_loss + a_cycle_loss + b_cycle_loss + a_idt_loss + b_idt_loss #total loss
                gen_loss.backward()
                self.g_optimizer.step() #generator update

                set_grad([self.Da, self.Db], True) # compute for disc'mr
                ...
                a_fake, b_fake = utils.cuda([a_fake, b_fake]) # saveing a sample from prev. generations

                
                a_real_dis = self.Da(a_real) # Disc'mr forward-pass
                a_fake_dis = self.Da(a_fake)
                ...
                a_dis_real_loss = self.MSE(a_real_dis, real_label) # Discriminator losses
                ...
                a_dis_loss.backward()
                b_dis_loss.backward()
                self.d_optimizer.step() # update of discriminators

                print("Epoch: (%3d) (%5d/%5d) | Generator Loss:%.2e | Discriminator Loss:%.2e" % 
            ...
            self.d_lr_scheduler.step() # lr updating
